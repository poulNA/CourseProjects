#include "floorplan.h"
/*\|||||||||||||||||||||||||||||||||||||||INTRO|||||||||||||||||||||||||||||||||||||||||||||/
This program we worked on functions is_leaf_node, is_internal_node, is_in_subtree, rotate,
recut, swap_module, swap swap_topology, get_expression, init_slicing_tree, get_total_resource.
is_leaf_node we checked for left and right children. is_internal_node we also checked for left
and right children but returned opposite. is_in_subtree we checked parsd upward using ->parent
to meet the upper node. rotate we switched width and height. recut we switched cutlines.
swap_module we used swap algorithm to swap modules. swap_topology we swapped the positions of
the nodes. We had to make sure one was the subtree of another. Then we had to check which side
to swap and then performed swap. For get_expression, we had to complete postfix_traversal.
This required parsing through the tree in post order and store the resource and cutline of each
node in expression array. To "store" we just set the cutline and module appropriately for expression.
For init_slicing_tree, we had to generate a left skewed tree given the module array and module size.
We had a base case of the left leaf node. This would be when level = amount of internal nodes. And
only in the base case would we ourselves create the left node. The recursive case would be creating
the current node and its right child node. We would recursively call for the left child of current
node. All modules that were set were from the module array and all internal nodes were set to cutline
V. 
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  if (ptr->module != NULL) {
    expression[n].cutline = UNDEFINED_CUTLINE;
    expression[n].module = ptr->module;
  }
  if (ptr->cutline != UNDEFINED_CUTLINE) {
    expression[n].cutline = ptr->cutline;
    expression[n].module = NULL;
  }
  (*nth)++;
}
...
int get_total_resource(node_t* ptr)
{
  ...
  int currentRes = 0;
  if (mod != NULL) {
    currentRes = mod->resource;
  }
  return currentRes + get_total_resource(ptr->left) + get_total_resource(ptr->right);
}
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  int iNodes = num_modules - 1;
  //BASE
  if (n == iNodes) {
    node->module = &(modules[n]);
  ...
  node_t* nodeR = (node_t*) malloc(sizeof(node_t));

  // Right Node
  nodeR->module = &(modules[n]);
  nodeR->cutline = UNDEFINED_CUTLINE;
  nodeR->parent = node;
  nodeR->left = NULL;
  nodeR->right = NULL;
