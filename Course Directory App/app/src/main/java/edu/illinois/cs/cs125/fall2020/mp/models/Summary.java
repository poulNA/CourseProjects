...
public class Summary implements SortedListAdapter.ViewModel {
  ...
  /**
   * Get the fullString for this Summary.
   *
   * @return the fullString for this Summary
   */
  public final String getFullString() {
    fullString = department + " " + number + ": " + title;
    return fullString;
  }
  ...
  /** Orders courseModels lexicographically for department, number, and title. */
  ...
  /**
   * Filters the courses in the list that contains the text.
   *
   * @param courses List of courses with summary type
   * @param text String to search for
   * @return returns a new list of courses that contain the text
   */
  public static List<Summary> filter(
      @NonNull final List<Summary> courses, @NonNull final String text) {
    List<Summary> returns = new ArrayList<>();
    for (int i = 0; i < courses.size(); i++) {
      if (courses.get(i).getFullString().toLowerCase().contains(text.toLowerCase())) {
        returns.add(courses.get(i));
      }
    }
    return returns;
  }
